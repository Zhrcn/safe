...
const SheetTrigger = React.forwardRef(({ className, children, ...props }, ref) => {
  if (React.Children.count(children) !== 1) {
    throw new Error("SheetTrigger must have exactly one child element")
  }
...
const SheetContent = React.forwardRef(({ className, side = "right", ...props }, ref) => (
  <SheetPrimitive.Portal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 bg-background text-foreground border border-border shadow-xl transition ease-in-out",
        className,
        sheetVariants({ side })
      )}
      {...props}
